...
void EB_IMU::setConfig(String msg)
{
  ...
  switch(act)
  {
  	case IMU_DATA_OFF:		// DATA_OFF
 	  	isMeasure = false;
			...
			LogPrintln(" LG] MN_RCV ImuData OUT_OFF by AP CMD");
		#endif
		break;
	...
	case IMU_DATA_ON:		// DATA ON
    	isMeasure = true;
		#if (LOG_MAIN_IMU_ONOFF)
			LogPrintln(" LG] MN_RCV ImuData OUT_ON by AP CMD");
		#endif
  ...
  #if LOG_IMU_CONFIG
  	LogPrintln(" LG] Rod Config Set m:" + msg);
  #endif
}
  ...
  for (n = 0; n < rbytes; n++)
  {
    ...
    if (mBuf[mBuf_cnt] == 0x0A)			// End Char [ LF(0x0A) ] of IMU OutData ? 
    {
...
#if 0
      #if (LOG_IMU_RECV_DATA_OUT)	// for Debug
		LogPrintln(" LG] imuData Recv : " + imuDataStr);	// IMU String LOG OUT
	  #endif
	  ...
	  if(NULL != addr)
	  {
      	...
      	{
        	item[i] = atof(addr);			// Change to Double(Float) from STRING_FLOAT
			// Next String Data Load
        	addr = strtok(NULL, ",");
			if(NULL == addr)
			{
			    #if (LOG_IMU_RECV_DATA_OUT)   // for Debug
				  if(i < (number_of_item-1))
				  {
					//result = IMU_PARSER_NG;
						// Nth Data Processing NG
				  		LogPrintln(" LG] ImuPar nTh Data NG: " + String(i));
				  }
				#endif
	  ...
	  else
	  {
		result = IMU_PARSER_NG;
		#if (LOG_IMU_RECV_DATA_OUT)   // for Debug
	  		// 1st  Data Processing NG
	  		LogPrintln(" LG] ImuPar 1st Data NG");
		#endif
	  }
	#endif
...
int EB_IMU::uartRecv_Normal()
{
  ...
  if (IMU_PARSER_OK == ebimu_parser(euler, IMU_DATA_SIZE))
  {
    ...
    if (true == isMeasure)		// Measure
    {
    	...
    	if (_imuCallback != nullptr)
    	{
    		#if 0
      		//_imuCallback(STR_IMU_EULER, String(euler[0]) + "," + String(euler[1]) + "," + String(euler[2]));
      		String msg = String(euler[0]) + "," + String(euler[1]) + "," + String(euler[2]) + ","
      		           + String(euler[3]) + "," + String(euler[4]) + "," + String(euler[5]);
      		_imuCallback(STR_PID_IMU_DATA, msg);
			#else
				_imuCallback(STR_PID_IMU_DATA, imuDataStr);
			#endif
    	}
    }
	else
...
int EB_IMU::uartRecv_Setup()
{
  ...
  if (IMU_PARSER_OK == ebimu_parser(euler, IMU_DATA_SIZE))		// Save euler[0]~[5] 6 items IMU Data = IMU Recv Data
  {
    ...
    if (isMeasure)			// IMU DATA OUT ON?
    {
		...
		#if 0
    	String str = "$09" + String(euler[0]) + "," + String(euler[1]) + "," + String(euler[2])
        	         + "," + String(euler[3]) + "," + String(euler[4]) + "," + String(euler[5]) + "%";
		#else
...
#if 0
	// Response to MAIN, imuDataCallback
  if (_imuCallback != nullptr)
  {
    _imuCallback(IMU_SETUP, str);
  }
