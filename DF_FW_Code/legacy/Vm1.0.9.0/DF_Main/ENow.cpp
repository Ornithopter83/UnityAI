...
void ENow::init(esp_now_recv_cb_t cbfunc)
{
	...
	#if 1	//=> 삭제(불필요), 이유: 타켓주소가 없으면 ROD끊김 장애 발생함.
...
void ENow::write(String pid, String str)
{
	...
	if(pid.equals(STR_PID_ROD_ALIVE_CHK)) { sendSeqNo++; }	  // test Send SeqNo
...
String ENow::getTargetAddress()
{
  ...
  //if (file && file.isDirectory())
  {
	...
	if(length)
	{
		...
		// TBD check inValid Addr
		//if(addr == "00:00:00:00:00:00")
		//{
		//	addr = "FF:FF:FF:FF:FF:FF";
		//}
	}
	LogPrintln(" LG] T_ADR length:" + String(length) + ",addr: " + addr);
