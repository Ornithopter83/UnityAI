...
#if (CPU_HARD_RESET_SUPPORT)
	#define RESET_PIN	EN			// RESET
#endif
#define BOOT_MODE_PIN		0		// IO_0

// Define Servo Moto
#if(IO_SERVO)
#define SERVOPWM_PIN 	3	// PIN Setting
#else
...
// i2c
//#define ANG_HOMESNS_PIN 	8	// I2C SDA Default
//#define RSV01_PIN		9		// I2C SCL Default
...
//#define PC_USB5V_PIN	35
//#define WDT_TRIG_PIN	36
...
//#define LOG_TX_PIN  	43		// LOG_TX : TXD_0
//#define LOG_RX_PIN  	44		// LOG_RX : RXD_0
#define UART1_TX_PIN  	43		// LOG_TX : TXD_0
#define UART1_RX_PIN  	44		// LOG_RX : RXD_0
//#define LED_OPT_R_PIN	43
//#define LED_OPT_L_PIN	44

//#define ILKDOOR_PIN		45
//#define SEL_BOARD_PIN 	46	// LOW-Main, HIGH-Slave

#define STRAPP_PSRAM_45	45
#define STRAPP_PSRAM_46	46

//#define I2C_CLK_PIN		47
//#define I2C_DT_PIN		48
//#define I2C_CLK_PIN		48
//#define I2C_DT_PIN		47
//
//#define PC_USB5V_PIN	47


#define DF_CONFIG_PIN		1		//	LM-JIG or NORMAL구분

#if (CONF_LM_JIG)
#define LM_24VON_PIN		DF_CONFIG_PIN	//	1
...
#else
  #if (IO_EXTIO_UART_PIN)	// NEW
	// Define Debug Serial Port
	#define UART2_RXD_PIN	1		// RXD_2
	#define UART2_TXD_PIN	2		// TXD_2
  //#else				// OLD
 	//	//#define LOG_TX_PIN  	2		// LOG_TX : IO_2( 38 pin)
	//#define LED_OPT_R_PIN	1
	//#define LED_OPT_L_PIN	2
  #endif
...
#if 0	// NOT_UESE
// LED PWM Channel
//#define CENT_R_CH PWM_CH1
#define CENT_G_CH PWM_CH2
#define CENT_B_CH PWM_CH3

#define BTM_CH 	  PWM_CH4
#define LEFT_CH   PWM_CH5
#define RIGHT_CH  PWM_CH6
#endif
...
#else				// 16W NEW BLDC
#define BLDC_DUTY_FULL		(1)			// 최종 REL VER
...
/*
#if (!MC_CONF_NO_4)		// #1,#2
#define BBN_BLDC_24VON  LOW
#define BBN_BLDC_24VOFF HIGH

//--- BOBBIN ACT LEVEL,  2024/6/18 ON Active Invert
#define BBN_MOT_ON	LOW
#define BBN_MOT_OFF	HIGH
...
//#if MC_CONF_NO_3	// #3(수작업 케이블 기계) 없어짐
  #if BBN_MOT_38W	// OLD 38W-MotorBank
	#define BBN_BLDC_24VON  HIGH
	#define BBN_BLDC_24VOFF LOW

	//--- BOBBIN ACT LEVEL,  2024/6/18 ON Active Invert
	#define BBN_MOT_ON	LOW
	#define BBN_MOT_OFF	HIGH

	#define BBN_MOT_CW	LOW
	#define BBN_MOT_CCW	HIGH

	#define BBN_PWM_INVERT	  (0)	  // INVERT BBN PWM DUTY

  #else	// NEW 16W-Mnyx
	#define BBN_BLDC_24VON  LOW		//HIGH	//LOW
	#define BBN_BLDC_24VOFF HIGH	//LOW	//HIGH

	//--- BOBBIN ACT LEVEL,  2024/9/23 ON Active Invert
	//--- BOBBIN ACT LEVEL,  2024/9/24 ON Active Invert
	#define BBN_MOT_ON	HIGH // LOW	// HIGH
	#define BBN_MOT_OFF	LOW // HIGH // LOW

  	#if (MC_CONF_NO_4)		// Proto2
	  #define BBN_MOT_CW	HIGH
	  #define BBN_MOT_CCW	LOW
  	#else
	  #define BBN_MOT_CW	LOW //HIGH
	  #define BBN_MOT_CCW	HIGH //LOW
  	#endif

  	#if (MC_CONF_NO_4)		// Proto2
	  #define BBN_PWM_INVERT	  (1) //(1) //(0)	  // INVERT BBN PWM DUTY
  	#else
	  #define BBN_PWM_INVERT	  (0) //(1) //(0)	  // INVERT BBN PWM DUTY
  	#endif
  #endif
#endif
*/
...
//#define DEFAULT_TORQ_30W_BELT 		64 // 65  // BLDC BELT Follow 
//#define DEFAULT_TORQ_30W_BELTSPRING 	64 // 65  // BLDC BELT Follow 
...
// 기본값 상수(INT)
	...
	//#define DEFAULT_TORQ	DEFAULT_TORQ_30W_GEAR		//TEST
  #if (BBN_SPRING_AUTOTENSION)
	#define DEFAULT_TORQ	DEFAULT_TORQ_30W_BELTSPRING		//TEST
  #else
	#define DEFAULT_TORQ	DEFAULT_TORQ_30W_BELT		//TEST
  #endif
#else	// 10W
	#if(CONFIG_TORQ_HUD)
		#define DEFAULT_TORQ	DEFAULT_TORQ_10W_HUD	//HUD 70
	#else
		#define DEFAULT_TORQ	DEFAULT_TORQ_10W		//TEST
	#endif
#endif
...
#define DEVCHK_BBN_DEFAULT_PWR		20	// 16 W
...
typedef	enum
{
	...
	GAME_WAIT,			// 결제전 (대기)
	GAME_STBY,			// 결재후, 각 항목 선택중
	CAST_WAIT,			// 게임시작되어 캐스팅 대기화면
	CAST_COMP,
	GAME_WAVE,
	GAME_BITE,
	GAME_HIT,
...
//--- PC(SERIAL_0) Macro====
#if 0				// (V108)
#define ApPrint		Serial.print
#define ApPrintf	Serial.printf
#define ApPrintln	Serial.println
// Define Macro
#define Resp2ApPrintln Serial.println		//확인용 RESP
#define StsSendPrintln Serial.println
#else
...
//--- LOG(SERIAL_1) Macro====
#define LogSerial	Serial2
#define LogPrint	Serial2.print
#define LogPrintf	Serial2.printf		// Not USE
#define LogPrintln	Serial2.printf("%10d",curr_ms_tick);Serial1.println

  #if (IO_EXTIO_UART_PIN)
// EXT_IO UART MACRO
#define ExtSerial	Serial1
#define send2extPrint	Serial1.print
#define send2extPrintf	Serial1.printf		// Not USE
#define send2extPrintln	Serial1.println
  #endif
#endif
...
//#define LED_MAX_DUTY	1023		// Duty 10 bit (1023)
...
#define MAC_ADDR_OK_STR_LEN	17

//---------------------------------------------
// === Define 	NOW PID	MAIN 2 ROD
//---------------------------------------------
//=== STRING PID, MAIN => ROD
//#define PID_ROD_ALIVE_CHK		1
//#define PID_PWR_1STCMD		5		// MAIN POWER ON SEND at MAIN RESET : Rod RESP
#define STR_PID_PWR_1STCMD		"05"		// MAIN POWER ON SEND at MAIN RESET : Rod RESP
//#define PID_ROD_INFO_REQ		6		// ROD INFO REQ SEND at AP Program START : Rod RESP INFO ( IMU Conntion & Bat Lvl )
#define STR_PID_ROD_INFO_REQ	"07"		// ROD INFO REQ SEND at AP Program START : Rod RESP INFO ( IMU Conntion & Bat Lvl )
//#define PID_ROD_SLEEP_CHK		10		// SLAVE SLEEP CHECK
//#define PID_ROD_ALIVE_CHK		10		// SLAVE ALIVE
#define STR_PID_ROD_SLEEP_CHK	"09"		// SLAVE SLEEP CHECK
#define STR_PID_ROD_ALIVE_CHK	"09"		// SLAVE ALIVE

//#define STR_PID_ROD_CTRL_NUM	"11"
#define STR_PID_ROD_BREAK		"11"
#define STR_PID_IMU_DATA_OUT	"13"		// NORMAL
#define STR_PID_IMU_CONF_SET	"13"		// SETUP MODE만

#define STR_PID_REEL_VRT_CONT	"19"		// ROD보드 V2만
#define STR_PID_ROD_VER_READ    "21"		// Slave & IMU Version Read
#define STR_PID_BTN_LED_CONT	"27"		// ROD보드 V2만

#define STR_PID_REEL_OUT_ALL_OFF	"29"		// ROD보드 V2만
//#define PID_MAIN_ADDR_WRITE	31		// WRITE ENT
#define STR_PID_MAIN_ADDR_WRITE	"31"		// WRITE ENT

#define STR_PID_AP_INFO_SEND	"33"		//AP INFO
#define STR_PID_SLEEP_ENABLE	"34"		//ROD Sleep Enable

//=== RESP/STS , MAIN <= ROD
#define PID_ROD_BOARD_TYPE		2		// ROD 보드 Type ( 000 - OLD/IMU Vertical, 001- New /IMU Horizontal)
#define PID_ROD_ALIVE_RESP		10		// SLAVE ALIVE
//#define PID_ROD_CTRL_NUM		11		// Pass to PC from SLAVE Control PID
//#define PID_IMU_EULER_NUM		13		// Save IMU(Jyro Data)
#define PID_IMU_DATA_NUM		14		// Send to PC
#define PID_ROD_BUTTON			16		// 
#define PID_ROD_ENC_CNT			18		// 
//#define PID_ROD_VER_RESP_ROD    19
#define PID_ROD_VER_RESP    	22		// ROD + IMU
#define PID_STS_IMU_CONNECT     24		// IMU CONNECT Status(00-Not Connect, 11-Connect)
//#define PID_STS_ROD_SLEEP_ENT   22		// SLAVE SLEEP ENTERENCE
#define PID_BATT_LVL			26
#define PID_ROD_ADDR_WRITE		32		// WRITE ENT
#define PID_ROD_SLPRSP			34		// Sleep/Wakeup Resp 

#if (0)	// NOT_USE
	#define PID_ROD_INFO_RESP		34		// ROD INFO
#endif


#if 0
#define PID_ROD_DIAG_IN     31		// SLAVE Diag IN
#define PID_ROD_DIAG_OUT    32		// SLAVE Diag OUT
#define PID_ROD_DIAG_FUNC   33		// SLAVE Diag FUNC
#define PID_ROD_DIAG_SET    34		// SLAVE Diag SET
#endif
...
//#define WRIE_MAX_DIST	(4*1000)	// 400 cm =  4미터
...
//#define MAIN_BOARD_V00_UNKNOWN	-1
